...
- הסביבה שלך צריכה להיות מוגדרת עבור קהל יעד עיקרי של **צרכנים בודדים**.
...
1. בשלב **מסנני מוצרים**, בחר בין האפשרויות הבאות.
...
1. בדף התוצאות קיימים חמישה מקטעי נתונים עיקריים:
    ...
    להלן הגורמים שיכולים להשפיע על המלצת מוצר שנוצרה על-ידי המודל.
        - **עסקאות עבר**: דפוסי הרכישה בעבר שימשו את המודל כדי ליצור המלצות על מוצרים. לדוגמה, המודל יכול להמליץ על *עכבר Surface Arc* אם מישהו רכש לאחרונה *Surface Book 3* ו- *Surface Pen*. המודל למד שמבחינה היסטורית, לקוחות רבים רכשו *עכבר Surface Arc* לאחר רכישת *Surface Book 3* ו- *Surface Pen*.
